...
use App\Http\Controllers\User\MyReferralStatusController;
...
Route::prefix('user')->name('user.')->group(function () {
    ...
    // My Status
    Route::middleware('referral_setting')->group(function () {
        Route::controller(MyReferralStatusController::class)->prefix('refer/level')->name('refer.level.')->group(function () {
            Route::get('/', 'index')->name('index');
    ...
    // cable services
    // Route::prefix('cabletv')->name('cabletv.')->group(function () {
    //     Route::get('/', [BillPayController::class, 'cableServices'])->name('index');
    //     // Show all plans for a TV service (DSTV, GOTV, Startimes, Showmax)
    //     Route::get('/{service}', [BillPayController::class, 'showPlans'])->name('plans');

    //     // Verify Smartcard
    //     Route::post('/{service}/verify', [BillPayController::class, 'verifySmartCard'])->name('verify');

    //     // Buy subscription
    //     Route::post('/{service}/buy', [BillPayController::class, 'buySubscription'])->name('buy');

    //     // Query transaction
    //     Route::post('/{service}/query', [BillPayController::class, 'queryTransaction'])->name('query');
    // });

    // // Data Services
    // Route::prefix('data')->name('data.')->group(function () {
    //     Route::get('/', [BillPayController::class, 'dataServices'])->name('index');
    //     Route::get('/{service}', [BillPayController::class, 'showDataPlans'])->name('plans');
    //     Route::post('/{service}/verify', [BillPayController::class, 'verifyNumber'])->name('verify');
    //     Route::post('/{service}/buy', [BillPayController::class, 'buyDataSubscription'])->name('buy');
    //     Route::get('/{service}/success', [BillPayController::class, 'success'])->name('success');
    // });
    // add money
    Route::get('/ck-bill-pay', [BillPayController::class, 'ckHome'])->name('ck.home');

    Route::prefix('data')->name('data.')->group(function () {
        Route::get('/', [BillPayController::class, 'ckData'])->name('index');

        Route::get('/data-plans/network/{id}', [BillPayController::class, 'getPlans'])
            ->name('dataNetwork');
        Route::post('/buy', [BillPayController::class, 'buyData'])->name('buy');
        // After purchase success page
        Route::get('/data/success/{id}', [BillPayController::class, 'dataSuccessPage'])->name('success');

        // PDF Receipt
        Route::get('/data/receipt/{id}/pdf', [BillPayController::class, 'dataReceiptPdf'])->name('receipt.pdf');
    ...
    Route::prefix('cable')->name('cable.')->group(function () {
        Route::get('/', [BillPayController::class, 'ckCableTv'])->name('index');
        Route::get('/cable-plans/provider/{id}', [BillPayController::class, 'getPackages'])->name('cableProviders');
        Route::post('/subscribe', [BillPayController::class, 'buyCableTv'])->name('subscribe');
    });
    ...
    Route::middleware('module:add-money')->group(function () {
        Route::controller(AddMoneyController::class)->prefix('add-money')->name('add.money.')->group(function () {
            ...
            Route::get('redirect/form/{gateway}', 'redirectUsingHTMLForm')->name('payment.redirect.form')->withoutMiddleware(['auth', 'verification.guard', 'user.google.two.factor', 'sms.verification.guard']);
            // redirect with Btn Pay
            Route::get('redirect/btn/checkout/{gateway}', 'redirectBtnPay')->name('payment.btn.pay')->withoutMiddleware(['auth', 'verification.guard', 'user.google.two.factor', 'sms.verification.guard']);
            Route::post('callback/response/{gateway}', 'callback')->name('payment.callback')->withoutMiddleware(['web', 'auth', 'verification.guard', 'user.google.two.factor', 'sms.verification.guard']);
            ...
            Route::post('success/response/{gateway}', 'postSuccess')->name('payment.global.success')->withoutMiddleware(['auth', 'verification.guard', 'user.google.two.factor', 'sms.verification.guard']);
            Route::post('cancel/response/{gateway}', 'postCancel')->name('payment.global.cancel')->withoutMiddleware(['auth', 'verification.guard', 'user.google.two.factor', 'sms.verification.guard']);
        });
    });
    // withdraw money
    ...
    Route::middleware('module:virtual-card')->group(function () {
        ...
        Route::middleware('virtual_card_method:flutterwave')->group(function () {
            Route::controller(VirtualcardController::class)->prefix('virtual-card')->name('virtual.card.')->group(function () {
                Route::get('/', 'index')->name('index');
                Route::get('create', 'createPage')->name('create.page')->middleware('kyc.verification.guard');
        ...
        Route::middleware('virtual_card_method:strowallet')->group(function () {
            Route::controller(StrowalletVirtualController::class)->prefix('strowallet-virtual-card')->name('strowallet.virtual.card.')->group(function () {
                ...
                Route::get('details/{card_id}', 'cardDetails')->name('details');
                Route::get('transaction/{card_id}', 'cardTransaction')->name('transaction');
                Route::get('webhook/transaction/{card_id}', 'webhookTransaction')->name('webhook.transaction');
                Route::put('change/status', 'cardBlockUnBlock')->name('change.status');
                Route::post('make/default/remove/default', 'makeDefaultOrRemove')->name('make.default.or.remove');

                Route::post('webhook', 'getWebhookData')->name('webhook')->withoutMiddleware(['auth', 'verification.guard', 'user.google.two.factor', 'sms.verification.guard', 'virtual_card_method']);
            });
        });
    ...
    Route::middleware('module:remittance-money')->group(function () {
        Route::controller(RemittanceController::class)->prefix('remittance')->name('remittance.')->group(function () {
            Route::get('/', 'index')->name('index');
    ...
    // Money Exchange
    Route::middleware('module:money-exchange')->group(function () {
        // money exchange
    ...
    Route::middleware('module:gift-cards')->group(function () {
        Route::controller(GiftCardController::class)->prefix('gift-card')->name('gift.card.')->group(function () {
            ...
            Route::post('webhook', 'webhookInfo')->name('webhook')->withoutMiddleware(['web', 'auth', 'verification.guard', 'user.google.two.factor', 'sms.verification.guard']);
        });
    });
    // transactions
...
Route::get('/test/monnify', [RegisterController::class, 'showForm'])->name('monnify.form');
Route::post('/test/monnify', [RegisterController::class, 'verifyNINBVN'])->name('monnify.verify');
