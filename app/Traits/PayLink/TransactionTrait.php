...
trait TransactionTrait
{
    ...
    public function notificationMerchant($output)
    {
        ...
        $notification_content = [
            ...
            'message' => 'Your Wallet'.' ('.$output['receiver_wallet']->currency->code.') '.'balance has been added'.' '.$output['charge_calculation']['receiver_amount'].' '.$output['receiver_wallet']->currency->code.' By '.$output['currency']->name,
            'time' => Carbon::now()->diffForHumans(),
    ...
    public function adminNotificationPayLink($trx_id, $output, $status, $user)
    {
        ...
        $notification_content = [
            ...
            'push_content' => __('web_trx_id').': '.$trx_id.','.__('Balance Added To Wallet').' ('.$output['receiver_wallet']->currency->code.') '.get_amount($output['charge_calculation']['receiver_amount'], $output['receiver_wallet']->currency->code).' '.'by'.','.$output['currency']->name.',('.$user->username.')',

            // admin db notification
            'notification_type' => NotificationConst::PAY_LINK,
            'trx_id' => $trx_id,
            'admin_db_title' => 'Pay Link'.' ('.$output['user_type'].')',
            'admin_db_message' => 'Transaction ID'.': '.$trx_id.','.'Balance Added To Wallet'.' ('.$output['receiver_wallet']->currency->code.') '.get_amount($output['charge_calculation']['receiver_amount'], $output['receiver_wallet']->currency->code).' by '.$output['currency']->name.',('.$user->username.')',
        ];
