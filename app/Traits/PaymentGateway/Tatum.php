...
trait Tatum
{
    ...
    public function tatumInit($output = null)
    {
        ...
        if (! $crypto_asset || ! $crypto_active_wallet) {
            throw new Exception(__('Gateway is not available right now! Please contact with system administration'));
        }
    ...
    public function tatumUserTransactionRequirements($trx_type = null)
    {
        ...
        if ($trx_type) {
            if (! array_key_exists($trx_type, $requirements)) {
                throw new Exception(__('User Transaction Requirements Not Found!'));
            }
    ...
    public function getTatumWallets($gateway, $request_credentials)
    {
        ...
        foreach ($new_coins as $coin) {
            if ($this->tatumRegisteredChains($coin)['status'] != true) {
                continue;
            }
    ...
    public function tatumRegisteredEndpoints($endpoint_key = null)
    {
        ...
        if ($endpoint_key) {
            if (! array_key_exists($endpoint_key, $generated_endpoints)) {
                throw new Exception("Provided endpoint key [$endpoint_key] not registered");
            }
    ...
    public function tatumRegisteredChains($coin = null)
    {
        ...
        if ($coin) {
            if (! array_key_exists($coin, $register_coins)) {
                throw new Exception("Provided coin [$coin] not registered in tatum");
            }
    ...
    public function senderAddressIsTatumRegistered($gateway, $coin, $address)
    {
        ...
        if (in_array($address, $credentials)) {
            return true;
        }
