...
{
    use Bkash,CoinGate,FlutterwaveTrait,Manual,PagaditoTrait,Paypal,PaystackTrait,PerfectMoney,RazorTrait,SslcommerzTrait,Stripe,Tatum;
    ...
    protected $wallet_currency_input_name = 'wallet_currency';
    ...
    public function gateway()
    {
        ...
        if (empty($request_data)) {
            throw new Exception(__('Gateway Information is not available. Please provide payment gateway currency alias'));
        }
        ...
        $user_wallet = $this->getUserWallet($validated[$this->wallet_currency_input_name]);
        if (! $user_wallet) {
            throw ValidationException::withMessages([
                $this->wallet_currency_input_name = __('User wallet not found!'),
            ]);
        }
    ...
    public function validator($data)
    {
        return Validator::make($data, [
            ...
            $this->wallet_currency_input_name => 'required|string|exists:currencies,code',
        ]);
    }
    ...
    public function getUserWallet($wallet_currency)
    {
        ...
        if ($this->predefined_user_wallet) {
            return $this->predefined_user_wallet;
        }
        ...
        $user_wallet = $wallet_model::where($column_name, $user_id)->whereHas('currency', function ($q) use ($wallet_currency) {
            $q->where('code', $wallet_currency);
        })->first();
    ...
    public function limitValidation($output)
    {
        ...
        if ($requested_amount < get_amount(($gateway_currency->min_limit / $exchange_rate), null, $output['amount']->wallet_precision) || $requested_amount > get_amount(($gateway_currency->max_limit / $exchange_rate), null, $output['amount']->wallet_precision)) {
            throw ValidationException::withMessages([
    ...
    public function amount()
    {
        ...
        $wallet_currency = $this->output['wallet'] ?? null;
        if (! $currency) {
            throw new Exception(__('Gateway currency not found'));
        }

        return $this->chargeCalculate($currency, $wallet_currency->currency);
    }
    ...
    public function chargeCalculate($currency, $receiver_currency = null)
    {
        ...
        $wallet_precision = get_wallet_precision($receiver_currency);

        $amount = get_amount($this->request_data[$this->amount_input], null, $wallet_precision);
        ...
        ($amount == '' || $amount == null) ? get_amount($amount, null, $wallet_precision) : get_amount($amount, null, $wallet_precision);
        ...
        $percent_charge_calc = get_amount(($amount / 100) * $percent_charges, null, $precision);

        $total_charge = get_amount($fixed_charge_calc + $percent_charge_calc, null, $precision);

        if ($receiver_currency) {

            $gCurrencyCode = $currency->currency_code;
            $gCurrencyRate = get_amount($sender_currency_rate, null, $precision);

            $wCurrencyCode = $receiver_currency->code;
            $wCurrencyRate = get_amount($receiver_currency->rate, null, $wallet_precision);
            ($wCurrencyRate == '' || $wCurrencyRate == null) ? $wCurrencyRate = 0 : get_amount($wCurrencyRate, null, $wallet_precision);

            $exchange_rate = get_amount(($gCurrencyRate / $wCurrencyRate), null, $precision);

            $data['exchange_rate'] = get_amount($exchange_rate, null, $precision);
            $data['requested_amount'] = get_amount($amount, null, $wallet_precision);

            $data['sender_cur_code'] = $gCurrencyCode;
            $data['sender_cur_rate'] = get_amount($gCurrencyRate, null, $precision);

            $data['receiver_cur_code'] = $wCurrencyCode;
            $data['receiver_cur_rate'] = get_amount($wCurrencyRate, null, $wallet_precision);

            $data['percent_charge'] = get_amount(($amount / 100) * $percent_charges * $exchange_rate, null, $precision) ?? 0;
            $data['fixed_charge'] = get_amount($fixed_charges, null, $precision) ?? 0;
            $data['total_charge'] = get_amount($data['percent_charge'] + $data['fixed_charge'], null, $precision);

            $data['total_amount'] = get_amount((($amount * $exchange_rate) + $data['total_charge']), null, $precision);

            $data['will_get'] = get_amount($amount, null, $wallet_precision);
            ...
            $exchange_rate = get_amount($default_currency->rate, null, $precision);
            ...
            $data = [
                ...
                'gateway_precision' => $precision,
                'wallet_precision' => $wallet_precision,
    ...
    public function render()
    {
        ...
        if (! is_array($output)) {
            throw new Exception(__('Render Failed! Please call with valid gateway/credentials'));
        }
    ...
    public function authenticateTempData()
    {
        ...
        if ($this->requestIsApiUser()) {
            ...
            if (! array_key_exists($creator_guard, $api_authenticated_guards)) {
                throw new Exception('Request user doesn\'t save properly. Please try again');
            }
        ...
        if (! $gateway_currency) {
            throw new Exception('Transaction Failed. Gateway currency not available.');
        }
        $requested_amount = $tempData['data']->amount->requested_amount ?? 0;
        $wallet_currency = $tempData['data']->amount->receiver_cur_code ?? 'USD';
        $validator_data = [
            $this->currency_input_name => $gateway_currency->alias,
            $this->amount_input => $requested_amount,
            $this->wallet_currency_input_name => $wallet_currency,
        ];
    ...
    public function responseReceive($type = null)
    {
        ...
        if ($this->requestIsApiUser()) {
            ...
            $api_authenticated_guards = PaymentGatewayConst::apiAuthenticateGuard();
            if (! array_key_exists($creator_guard, $api_authenticated_guards)) {
                throw new Exception(__("Request user doesn\'t save properly. Please try again"));
            }
            if ($creator_table == null || $creator_id == null || $creator_guard == null) {
                throw new Exception(__("Request user doesn\'t save properly. Please try again"));
            }
        ...
        $gateway_currency = PaymentGatewayCurrency::find($currency_id);
        if (! $gateway_currency) {
            throw new Exception(__('Transaction Failed. Gateway currency not available'));
        }
        $requested_amount = $tempData['data']->amount->requested_amount ?? 0;
        $wallet_currency = $tempData['data']->amount->receiver_cur_code ?? 'USD';
    ...
    public function requestIsApiUser()
    {
        ...
        if (request()->routeIs('api.*') || request()->routeIs('agent_api.*')) {
            return true;
        }
    ...
    public function api()
    {
        $output = $this->output;
        if (! $output) {
            throw new Exception(__(__('Something went wrong! Gateway render failed. Please call gateway() method before calling api() method')));
        }
    ...
    public function setUrlParams(string $url_params)
    {
        $output = $this->output;
        if (! $output) {
            throw new Exception(__('Something went wrong! Gateway render failed. Please call gateway() method before calling api() method'));
        }
    ...
    {
        $output = $this->output;
        if (! $output || ! isset($output['url_params'])) {
            $params = '';
        }
    ...
    public function setGatewayRoute($route_name, $gateway, $params = null)
    {
        if (! Route::has($route_name)) {
            throw new Exception('Route name ('.$route_name.') is not defined');
        }
        if ($params) {
    ...
    public function handleCallback($reference, $callback_data, $gateway_name)
    {
        ...
        if ($transaction) {
            ...
            $wallet_currency = $transaction->creator_wallet->currency->code ?? 'USD';
            $validator_data = [
                $this->currency_input_name => $gateway_currency->alias,
                $this->amount_input => $requested_amount,
                $this->wallet_currency_input_name => $wallet_currency,
    ...
    public function removeSpacialChar($string, $replace_string = '')
    {
    ...
    public function handleBtnPay($gateway, $request_data)
    {
        ...
        if (! array_key_exists('token', $request_data)) {
            throw new Exception('Requested with invalid token');
        }
