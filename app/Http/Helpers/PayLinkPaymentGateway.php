...
class PayLinkPaymentGateway
{
    ...
    public function payLinkGateway()
    {
        ...
        if (empty($request_data)) {
            throw new Exception('Gateway Information is not available. Please provide payment gateway currency alias');
        }
        ...
        if ($this->output['payment_link']->user != null) {
            $receiver_wallet = UserWallet::with('user', 'currency')->where('user_id', $this->output['payment_link']->user->id)->whereHas('currency', function ($q) use ($data) {
                $q->where('code', $data->currency)->active();
            })->active()->first();
            $userType = 'USER';
            $userGuard = 'web';
        } elseif ($this->output['payment_link']->merchant != null) {
            $receiver_wallet = MerchantWallet::with('merchant', 'currency')->where('merchant_id', $this->output['payment_link']->merchant->id)->whereHas('currency', function ($q) use ($data) {
                $q->where('code', $data->currency)->active();
            })->active()->first();
            $userType = 'MERCHANT';
            $userGuard = 'merchant';
    ...
    public function chargeCalculate($currency)
    {
        ...
        $sender_currency_code = $currency->currency_code;
        ...
        $receiver_fixed_charge = $currency->fixed_charge;
        $receiver_percent_charge = ($amount / 100) * $currency->percent_charge;
        ...
        if ($receiver_will_get <= 0) {
            throw new Exception(__('Transaction Failed, Please Contact With Support!'));
        }
        $data = [
            ...
            'sender_cur_code' => $sender_currency_code,
    ...
    public function render()
    {
        ...
        if (! is_array($output)) {
            throw new Exception('Render Failed! Please call with valid gateway/credentials');
        }
    ...
    public function authenticateTempData()
    {
        ...
        if (empty($tempData) || empty($tempData['type'])) {
            throw new Exception(__("Transaction Failed. Record didn\'t saved properly. Please try again"));
        }
    ...
    public function responseReceive($type = null)
    {
        $tempData = $this->request_data;
        if (empty($tempData) || empty($tempData['type'])) {
            throw new Exception('Transaction failed. Record didn\'t saved properly. Please try again.');
        }
    ...
    public function setUrlParams(string $url_params)
    {
        $output = $this->output;
        if (! $output) {
            return throw new Exception('Something went wrong! Gateway render failed. Please call gateway() method before calling api() method');
        }
    ...
    public function setGatewayRoute($route_name, $gateway, $params = null)
    {
        if (! Route::has($route_name)) {
            return throw new Exception('Route name ('.$route_name.') is not defined');
        }
        if ($params) {
