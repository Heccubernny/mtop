...
function get_all_timezones()
{
    $countries = json_decode(file_get_contents(resource_path('world/countries.json')), true);
    $timezones = [];
    foreach (data_get($countries, '*.timezones', []) as $item) {
        $timezones = array_merge($timezones, $item);
    }
...
function totalAdminProfits()
{
    ...
    $total_profits = agentOnBaseCurrency(TransactionCharge::with('transactions')
...
function userActiveCardData()
{
    if (virtual_card_system('flutterwave') == 'flutterwave') {
        ...
        $total_cards = VirtualCard::where('user_id', auth()->user()->id)->where('is_active', 1)->get();
    } elseif (virtual_card_system('sudo') == 'sudo') {
        ...
        $total_cards = SudoVirtualCard::where('user_id', auth()->user()->id)->where('status', 1)->get();
    } elseif (virtual_card_system('stripe') == 'stripe') {
        ...
        $total_cards = StripeVirtualCard::where('user_id', auth()->user()->id)->where('status', 1)->get();
    } elseif (virtual_card_system('strowallet') == 'strowallet') {
        ...
        $total_cards = StrowalletVirtualCard::where('user_id', auth()->user()->id)->where('is_active', 1)->get();
    }
    $virtual_card_info = [
        'virtual_cards' => $virtual_cards,
        'active_cards' => $active_cards,
...
function agentOnBaseCurrency($profits)
{
    $totalAmount = 0;
    foreach ($profits as $profit) {
        $requestAmount = $profit->total_charge;
        if ($profit->transactions->type == PaymentGatewayConst::TYPEADDMONEY) {
            $exchange_rate = $profit->transactions->currency->rate ?? get_default_currency_rate();
        } else {
...
function virtualCardAmountOnBaseCurrency($cards)
{
    ...
    foreach ($cards as $card) {
        $requestAmount = $card->amount ?? $card->balance ?? 0;
        $exchange_rate = Currency::where('code', $card->currency)->first()->rate ?? 1;
        $result = $requestAmount / $exchange_rate;
        $totalAmount += $result;
    }
...
{
    if (count($supported_currency) > 0) {
        $all_currency = Currency::active()->get();
        $filtered_currency = $all_currency->filter(function ($currency) use ($supported_currency) {
        ...
        $all_currency = Currency::active()->get();
        $all_currency->makeHidden(['updated_at', 'admin_id', 'editData', 'sender', 'both', 'receiver', 'senderCurrency', 'receiverCurrency', 'created_at']);

        return $all_currency->values() ?? [];
    }

}

function user_wallets($guard, $column, $limit = null)
{
    $guard_name = $guard['guard'];
    $user = $guard['user'];
    $walletModel = null;

    // Determine the wallet model based on the guard name
    switch ($guard_name) {
        case 'api':
            $walletModel = UserWallet::query();
            break;
        case 'merchant_api':
            $walletModel = MerchantWallet::query();
            break;
        case 'agent_api':
            $walletModel = AgentWallet::query();
            break;
        default:
            return [];
    }
    ...
    $walletModel->where($column, $user->id)
        ->active() // Ensures the wallet itself is active
    ...
    // Get the wallets and hide unnecessary fields
    $user_wallets = $walletModel->get();
    $user_wallets->makeHidden([
        'id', 'user_id', 'currency_id', 'created_at',
        'updated_at', 'merchant_id', 'agent_id',
    ]);
...
{
    return $gateway->crypto == 1 ? get_precision_from_admin()['crypto_precision_value'] : get_precision_from_admin()['fiat_precision_value'];
}
...
function removeLangJson($languageJsonCodes)
{
    $directory = base_path('lang'); // Path to the lang folder
    $filesToDelete = $languageJsonCodes;

    // Retrieve the language codes from the database
    $existingLanguageCodes = Language::pluck('code')->toArray();

    foreach ($filesToDelete as $file) {
        // Get the language code by removing ".json" from the filename
...
function showDateTime($date, $format = 'Y-m-d H:i:s', $timezone = null)
{
    try {
        $dt = new DateTime($date, new DateTimeZone(config('app.timezone')));
...
function active_billpay_provider()
{
    // Check ClubKonnect
    $ck = ReloadlyApi::where('provider', ReloadlyApi::PROVIDER_CLUBKONNECT)
        ->where('status', 1)
        ->where('type', ReloadlyApi::UTILITY)
        ->first();

    // Check Reloadly
    $reloadly = ReloadlyApi::where('provider', ReloadlyApi::PROVIDER_RELOADLY)
        ->where('status', 1)
        ->where('type', ReloadlyApi::UTILITY_PAYMENT)
        ->first();

    // logger()->info('ClubKonnect:', ['ck' => $ck]);
    // logger()->info('Reloadly:', ['reloadly' => $reloadly]);

    // Both cannot be active at the same time
    if ($ck && $reloadly) {
        logger()->warning('Both providers are active. This should not happen!');
        // return Redirect::back()->with('error', 'Invalid provider configuration.');
    ...
    // Neither provider is active (should not happen)
    logger()->warning('No active provider found.');
    // return Redirect::back()->with('error', 'No active bill pay provider found.');

    throw new \Exception('No active bill pay provider found.');
