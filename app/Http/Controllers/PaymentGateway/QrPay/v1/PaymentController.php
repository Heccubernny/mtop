...
class PaymentController extends Controller
{
    ...
    public function paymentCreate(Request $request)
    {
        ...
        if (! $access_token) {
            return Response::paymentApiError([__('Access denied! Token not found')], [], 403);
        }

        $request_record = PaymentOrderRequest::where('access_token', $access_token)->first();
        if (! $request_record) {
            return Response::paymentApiError([__('Requested with invalid token!')], [], 403);
        }
        ...
        if ($request_record->status == PaymentGatewayConst::EXPIRED) {
            return Response::paymentApiError([__('Request token is expired')], [], 401);
        }

        if ($request_record->status != PaymentGatewayConst::CREATED) {
            return Response::paymentApiError([__('Requested with invalid token!')], [], 400);
        }
        ...
        if ($validator->fails()) {
            return Response::paymentApiError($validator->errors()->all(), [], 400);
        }
        ...
        if (! $merchant || ! $developer_credentials) {
            return Response::paymentApiError([__("Merchant does't exists")]);
        }
        ...
        if (request()->is('*/sandbox/*')) {
            ...
            if ($developer_credentials->mode != PaymentGatewayConst::ENV_SANDBOX) {
                return Response::paymentApiError([__('Requested with invalid credentials')]);
            }
    ...
    public function paymentPreview($token)
    {
        ...
        $currency = Currency::where('code', $request_record->currency)->active()->first();
        ...
        if (request()->is('*/sandbox/*')) {
            ...
            $wallet = UserWallet::where('user_id', $user->id)->whereHas('currency', function ($q) use ($currency) {
                $q->where('code', $currency->code)->active();
            })->active()->first();
        ...
        return view('qrpay-gateway.pages.confirm', compact('page_title', 'merchant_configuration', 'request_record', 'user', 'payment_gateway_image', 'token', 'submit_form_url', 'gateway_setting', 'wallet', 'currency'));
    }
    ...
    public function paymentConfirm(Request $request, $token)
    {
        ...
        $charges = [
            ...
            'payable_currency' => $request_record->currency,
        ];
        ...
        if (request()->is('*/sandbox/*')) {
            ...
            if ($developer_credentials->mode != PaymentGatewayConst::ENV_PRODUCTION || $trx_type == PaymentGatewayConst::VIRTUAL) {
                if (virtual_card_system('flutterwave')) {
                    ...
                    $virtual_card = StrowalletVirtualCard::where('user_id', $user->id)->where('is_default', true)->first();
                    if (! $virtual_card) {
                        $page_title = 'Process Error';
                        $data = [
                            'title' => 'Virtual Card Not Found!',
                ...
                $card_balance = $virtual_card->amount ?? $virtual_card->balance;
                $charges['exchange_rate'] = $virtual_card->card_currency->rate / $user_wallet->currency->rate;
                $charges['payable'] = $request_record->amount * $charges['exchange_rate'];
                $charges['payable_currency'] = $virtual_card->currency;

                if ($charges['payable'] > $card_balance) {
                    $page_title = 'Process Error';
                    $data = [
            ...
            if ($trx_type == PaymentGatewayConst::WALLET) {
                ...
                if ($user_wallet->balance < $charges['payable']) {
                    $page_title = 'Process Error';
                    $data = [
                        'title' => 'Insufficient balance!',
            ...
            try {
                ...
                if ($trx_type == PaymentGatewayConst::WALLET) {
                    ...
                    $virtual_dynamic_field_name = virtual_card_system('strowallet') == 'strowallet' ? 'balance' : 'amount';
                    $virtual_card->$virtual_dynamic_field_name -= $charges['payable'];
                    $virtual_card->save();
                }
