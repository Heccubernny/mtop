...
class DemoCheckoutController extends Controller
{
    ...
    {
        $sender_wallets = Currency::active()->get();

        return view('qrpay-gateway.pages.checkout', compact('sender_wallets'));
    }

    // get token
    public function getToken()
    {
        $base_url = url('/').'/pay/sandbox/api/v1';
    ...
    public function initiatePayment(Request $request)
    {
        $validator = Validator::make($request->all(), [
            ...
            'wallet' => 'required|string|exists:currencies,code',
        ]);
        if ($validator->fails()) {
        ...
        try {
            ...
            $response = $client->request('POST', url('/').'/pay/sandbox/api/v1/payment/create', [
                'json' => [
                    'amount' => $validated['amount'],
                    'currency' => $validated['wallet'],
    ...
    public function custom_random_string($length = 10)
    {
