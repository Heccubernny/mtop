...
class AgentCareController extends Controller
{
    ...
    public function userDetails($username)
    {
        ...
        if (! $user) {
            return back()->with(['error' => ['Opps! Agent not exists']]);
        }
        $wallets = AgentWallet::where('agent_id', $user->id)
            ->whereHas('currency', function ($q) {
                $q->where('status', GlobalConst::ACTIVE); // Ensure the currency is active
            })
            ->paginate(16);
        $add_money_amount = amountOnBaseCurrency(Transaction::where('agent_id', $user->id)->where('type', PaymentGatewayConst::TYPEADDMONEY)->where('status', 1)->get());
        $money_out_amount = amountOnBaseCurrency(Transaction::where('agent_id', $user->id)->where('type', PaymentGatewayConst::TYPEMONEYOUT)->where('status', 1)->get());
        $total_transaction = amountOnBaseCurrency(Transaction::where('agent_id', $user->id)->where('status', 1)->get());
        $data = [
            'wallets' => $wallets,
    ...
    public function userDetailsUpdate(Request $request, $username)
    {
        ...
        if (! $user) {
            return back()->with(['error' => [__('Ops! Agent not exists')]]);
        }
    ...
    public function kycDetails($username)
    {
        $user = Agent::where('username', $username)->first();
        if (! $user) {
            return back()->with(['error' => [__("Ops! agent doesn't exists")]]);
        }
    ...
    public function kycApprove(Request $request, $username)
    {
        ...
        if ($user->kyc_verified == GlobalConst::VERIFIED) {
            return back()->with(['warning' => ['Agent already KYC verified']]);
        }
        if ($user->kyc == null) {
            return back()->with(['error' => [__('Agent KYC information not found')]]);
        }
