...
class SetupSectionsController extends Controller
{
    ...
    //     // Get all languages
    //     $languages = Language::get();
    //     $current_local = get_default_language_code(); // default language code
    //     $validation_rules = [];
    //     $language_wise_data = [];

    //     foreach ($request->all() as $input_name => $input_value) {
    //         foreach ($languages as $language) {
    //             // Split input name to get language code and field name
    //             $input_name_check = explode("_", $input_name);
    //             $input_lang_code = array_shift($input_name_check);
    //             $input_name_check = implode("_", $input_name_check);

    //             // Check if this input matches the language code
    //             if ($input_lang_code == $language['code']) {
    //                 if (array_key_exists($input_name_check, $basic_field_name)) {
    //                     $langCode = $language['code'];

    //                     // Set validation rule for the default language
    //                     if ($current_local == $langCode) {
    //                         $validation_rules[$input_name] = $basic_field_name[$input_name_check];
    //                     } else {
    //                         // Set nullable for non-default languages
    //                         $validation_rules[$input_name] = str_replace("required", "nullable", $basic_field_name[$input_name_check]);
    //                     }

    //                     // Store the data for each language
    //                     $language_wise_data[$langCode][$input_name_check] = $input_value;
    //                 }
    //                 break;
    //             }
    ...
    //     // Fill missing values for languages with null values
    //     foreach ($languages as $language) {
    //         $langCode = $language['code'];
    //         foreach ($language_wise_data[$langCode] as $field => $value) {
    //             // If the value is null, fill with the default language value
    //             if (is_null($value) && isset($language_wise_data[$current_local][$field])) {
    //                 // $language_wise_data[$langCode][$field] = $language_wise_data[$current_local][$field];

    //                 $language_wise_data[$langCode][$field] =  $this->translate_data($current_local,$language_wise_data[$current_local][$field], $langCode) ?? null ;
    //             }
    //         }
    //     }
    ...
    //     return $language_wise_data;
    // }
    // public function translate_data($default_lan,$text,$translate_lan){

    //     // URL for the API request
    //     $encoded_text = urlencode($text);
    //     $url = "https://script.google.com/macros/s/AKfycbzPV-DPYwkS7sMeVJoSzG-Eu9b7HewJqBLr5SRKZQ-zvgSfidbR_Povsy0LGMX7RkjJ/exec?text=$encoded_text&lang=$translate_lan";
