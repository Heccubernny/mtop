        ...
        try {
            ...
            if ($approved) {
                ...
                $notification_content = [
                    'title' => __('Bill Pay'),
                    'message' => 'Your Bill Pay request approved by admin '.get_amount($data->request_amount, $data->details->charges->wallet_currency).' & Bill Number is: '.@$data->details->bill_number.' successful.',
                ...
                if ($data->user_id != null) {
                    ...
                    $userWallet = AgentWallet::where('agent_id', $data->agent_id)->whereHas('currency', function ($q) use ($data) {
                        $q->where('code', $data->details->charges->wallet_currency ?? get_default_currency_code())->active();
                    })->active()->first();
    ...
    public function rejected(Request $request)
    {
        ...
        try {
            ...
            if ($rejected) {
                ...
                $notification_content = [
                    'title' => __('Bill Pay'),
                    'message' => 'Your Bill Pay request rejected by admin '.get_amount($data->request_amount, $data->details->charges->wallet_currency).' '.' & Bill Number is: '.@$data->details->bill_number,
                    'image' => files_asset_path('profile-default'),
                ];
