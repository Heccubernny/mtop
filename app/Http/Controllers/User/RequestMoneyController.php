...
class RequestMoneyController extends Controller
{
    ...
    public function submit(Request $request)
    {
        ...
        if (! $sender_wallet) {
            return back()->with(['error' => [__("Your wallet isn't available with currency").' ('.$validated['currency'].')']]);
        }

        $receiver_currency = Currency::active()->where('code', $validated['currency'])->first();
        ...
        $receiver = User::where(function ($query) use ($validated) {
            $query->where('email', $validated['credentials'])
                ->orWhere('mobile', $validated['credentials'])
                ->orWhere('full_mobile', $validated['credentials']);
        })->active()->first();
        if (! $receiver) {
        ...
        if ($receiver->username == $sender_wallet->user->username || $receiver->email == $sender_wallet->user->email) {
            return back()->with(['error' => [__("Can't Request Money To Your Own")]]);
        }
        ...
        // daily and monthly
        try {
            (new TransactionLimit)->trxLimit('user_id', $sender_wallet->user->id, PaymentGatewayConst::REQUESTMONEY, $sender_wallet->currency, $validated['request_amount'], $trx_charges, PaymentGatewayConst::SEND);
        } catch (Exception $e) {
        ...
        try {
            ...
            if ($basic_setting->email_notification == true) {
                ...
                $notifyDataSender = [
                    ...
                    'request_amount' => get_amount($charges['request_amount'], $charges['sender_currency'], $charges['precision_digit']),
                    'payable' => get_amount($charges['payable'], $charges['receiver_currency'], $charges['precision_digit']),
                    'charges' => get_amount($charges['total_charge'], $charges['receiver_currency'], $charges['precision_digit']),
                    'will_get' => get_amount($charges['request_amount'], $charges['sender_currency'], $charges['precision_digit']),
                    'status' => __('Pending'),
                ];
                try {
            ...
            // sender notifications Sms
            if ($basic_setting->sms_notification == true) {
                try {
                    // sender sms
                    sendSms($sender_wallet->user, 'REQUEST_MONEY', [
                        'amount' => get_amount($charges['request_amount'], $charges['sender_currency'], $charges['precision_digit']),
                        'receiver' => $receiver->username.'('.$receiver->fullname.')',
                        'trx' => $trx_id,
                ...
                try {
                    ...
                    sendSms($receiver, 'REQUEST_MONEY_RECEIVER', [
                        'amount' => get_amount($charges['request_amount'], $charges['sender_currency'], $charges['precision_digit']),
                        'requestor' => $sender_wallet->user->username.'('.$sender_wallet->user->fullname.')',
                        'trx' => $trx_id,
                        'time' => now()->format('Y-m-d h:i:s A'),
                    ]);
                } catch (Exception $e) {
    ...
    public function adminNotification($trx_id, $charges, $sender, $receiver)
    {
        ...
        $notification_content = [
            ...
            'email_content' => __('web_trx_id').' : '.$trx_id.'<br>'.__('sender').': @'.$sender->email.'<br>'.__('Receiver').': @'.$receiver->email.'<br>'.__('request Amount').' : '.get_amount($charges['request_amount'], $charges['sender_currency'], $charges['precision_digit']).'<br>'.__('Fees & Charges').' : '.get_amount($charges['total_charge'], $charges['sender_currency'], $charges['precision_digit']).'<br>'.__('Total Payable Amount').' : '.get_amount($charges['payable'], $charges['receiver_currency'], $charges['precision_digit']).'<br>'.__('Recipient Received').' : '.get_amount($charges['receiver_amount'], $charges['receiver_currency'], $charges['precision_digit']).'<br>'.__('Status').' : '.__('Pending'),

            // push notification
            'push_title' => __('Request Money Request Send').' '.__('Successful'),
            'push_content' => __('web_trx_id').' '.$trx_id.' '.__('sender').': @'.$sender->email.' '.__('Receiver').': @'.$receiver->email.' '.__('Sender Amount').' : '.get_amount($charges['request_amount'], $charges['sender_currency'], $charges['precision_digit']).' '.__('Receiver Amount').' : '.get_amount($charges['receiver_amount'], $charges['receiver_currency'], $charges['precision_digit']),

            // admin db notification
            'notification_type' => NotificationConst::REQUEST_MONEY,
            'trx_id' => $trx_id,
            'admin_db_title' => 'Request Money Request Send'.' '.'Successful'.' '.get_amount($charges['request_amount'], $charges['sender_currency'], $charges['precision_digit']).' ('.$trx_id.')',
            'admin_db_message' => 'Sender'.': @'.$sender->email.','.'Receiver'.': @'.$receiver->email.','.'Sender Amount'.' : '.get_amount($charges['request_amount'], $charges['sender_currency'], $charges['precision_digit']).','.'Receiver Amount'.' : '.get_amount($charges['receiver_amount'], $charges['receiver_currency'], $charges['precision_digit']),
        ];
    ...
    public function insertSenderCharges($id, $charges, $sender, $receiver)
    {
        ...
        try {
            ...
            $notification_content = [
                ...
                'message' => __('Request Money to').' '.$receiver->fullname.' '.get_amount($charges->request_amount, $charges->sender_currency, $charges->precision_digit).' '.__('Successful'),
    ...
    public function insertReceiverCharges($id, $charges, $sender, $receiver)
    {
        ...
        try {
            ...
            $notification_content = [
                'title' => __('request Money'),
                'message' => __('Request Money from').' '.$sender->fullname.' '.get_amount($charges->receiver_amount, $charges->receiver_currency, $charges->precision_digit).' '.__('Successful'),
    ...
    public function approved(Request $request)
    {
        ...
        if (! $sender_wallet) {
            return back()->with(['error' => [__("Sender wallet isn't available with currency").' ('.$sender_currency.')']]);
        }
        // request receiver
        ...
        if (! $receiver_wallet) {
            return back()->with(['error' => [__("Receiver wallet isn't available with currency").' ('.$receiver_currency.')']]);
        }
