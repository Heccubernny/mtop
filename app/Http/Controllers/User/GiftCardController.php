...
class GiftCardController extends Controller
{
    ...
    public function giftCardOrder(Request $request)
    {
        try {
            ...
            if (! $sender_country) {
                return back()->with(['error' => [__('Sender Country Is Not Valid')]]);
            }
            ...
            $receiver_country = Currency::where('code', $form_data['receiver_currency'])->first();
            if (! $receiver_country) {
                return back()->with(['error' => [__('Receiver Country Is Not Valid')]]);
            }
            $cardCharge = TransactionSetting::where('slug', 'gift_card')->where('status', 1)->first();
            ...
            if ($charges['payable'] > $userWallet->balance) {
                return back()->with(['error' => [__("You don't have sufficient balance")]]);
            }
