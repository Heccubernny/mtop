...
class DashboardController extends Controller
{
    ...
    public function index()
    {
        ...
        $data['totalReceiveRemittance'] = amountOnBaseCurrency(Transaction::auth()->remitance()->where('attribute', 'RECEIVED')->where('status', 1)->get());
        $data['totalSendRemittance'] = amountOnBaseCurrency(Transaction::auth()->remitance()->where('attribute', 'SEND')->where('status', 1)->orderBy('id', 'DESC')->get());
        $data['cardAmount'] = virtualCardAmountOnBaseCurrency(userActiveCardData()['total_cards']);
        ...
        $fiat_wallets = UserWallet::auth()->whereHas('currency', function ($q) {
            return $q->where('type', GlobalConst::FIAT)->where('status', GlobalConst::ACTIVE);
        })->limit(8)->get();

        $crypto_wallets = UserWallet::auth()->whereHas('currency', function ($q) {
            return $q->where('type', GlobalConst::CRYPTO)->where('status', GlobalConst::ACTIVE);
        })->limit(8)->get();

        //
        return view('user.dashboard', compact('page_title', 'baseCurrency', 'transactions', 'data', 'chartData', 'fiat_wallets', 'crypto_wallets'));
    }
