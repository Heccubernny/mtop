...
class SendMoneyController extends Controller
{
    ...
    public function confirmed(Request $request)
    {
        ...
        if (! $sender_wallet) {
            return back()->with(['error' => [__("Your wallet isn't available with currency").' ('.$validated['sender_wallet'].')']]);
        }

        if ($sender_wallet->user->email == $validated['credentials'] || $sender_wallet->user->mobile == (int) $validated['credentials'] || $sender_wallet->user->full_mobile == $validated['credentials']) {
            return back()->with(['error' => [__("Can't send money to your own")]]);
        }
        ...
        if (! $receiver_wallet) {
            return back()->with(['error' => [__('Receiver wallet not available')]]);
        }
