...
{
    use FlutterwaveTrait,Manual,RazorTrait,SslcommerzTrait,Stripe,TransactionAgent;
    ...
    public function index()
    {
        ...
        $wallet_currencies = Currency::whereIn('id', $user_wallets->pluck('currency_id')->toArray())->where('status', true)->orderByDesc('default')->get();
        ...
        return view('user.sections.add-money.index', compact('page_title', 'transactions', 'payment_gateways_currencies', 'wallet_currencies'));
    }
    ...
    public function success(Request $request, $gateway)
    {
        ...
        if (! $checkTempData) {
            return redirect()->route('user.add.money.index')->with(['error' => [__("Transaction Failed. The record didn't save properly. Please try again")]]);
        }
