...
class AuthorizationController extends Controller
{
    ...
    public function checkExist(Request $request)
    {
        ...
        $user = Merchant::where($column, $column_value)->first();
        if ($user) {
    ...
    public function sendOtp(Request $request)
    {
        ...
        if ($validated['register_type'] == GlobalConst::PHONE) {
            ...
            $code = generate_random_code();
            $mobile_code = remove_special_char($validated['mobile_code']);
            $mobile = $mobile = get_mobile_number($mobile_code, remove_special_char($validated['credentials']));
            $full_mobile = $mobile_code.$mobile;
            ...
            if ($exist || $exists_agent || $exists_merchant) {
                $message = ['error' => [__('User already  exists, please try with another phone number.')]];
    ...
    public function verifyEmailOtp(Request $request)
    {
        ...
        $auth_column = MerchantAuthorization::where('email', $request->email)->where('code', $code)->first();
        if (! $auth_column) {
    ...
    public function resendEmailOtp(Request $request)
    {
        ...
        $exists_agent = Agent::where('email', $validated['email'])->first();
        $exists_merchant = Merchant::where('email', $validated['email'])->first();
        if ($exist || $exists_agent || $exists_merchant) {
            $message = ['error' => [__('User already  exists, please try with another phone or email address..')]];
    ...
    public function verifySmsOtp(Request $request)
    {
        ...
        $code = $validated['code'];
        $otp_exp_sec = BasicSettingsProvider::get()->otp_exp_seconds ?? GlobalConst::DEFAULT_TOKEN_EXP_SEC;
        $auth_column = MerchantAuthorization::where('phone', $full_mobile)->where('code', $code)->first();
    ...
    public function resendSmsOtp(Request $request)
    {
        ...
        $resend = MerchantAuthorization::where('phone', $full_mobile)->first();
        if ($resend) {
            if (Carbon::now() <= $resend->created_at->addMinutes(GlobalConst::USER_VERIFY_RESEND_TIME_MINUTE)) {
                $message = ['error' => [__('You can resend the verification code after').' '.Carbon::now()->diffInSeconds($resend->created_at->addMinutes(GlobalConst::USER_VERIFY_RESEND_TIME_MINUTE)).' '.__('seconds')]];
        ...
        DB::beginTransaction();
        try {
            $oldToken = MerchantAuthorization::where('phone', $full_mobile)->get();
            if ($oldToken) {
                foreach ($oldToken as $token) {
                    $token->delete();
                }
