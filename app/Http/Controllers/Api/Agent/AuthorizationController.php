...
class AuthorizationController extends Controller
{
    ...
    public function checkExist(Request $request)
    {
        ...
        $user = Agent::where($column, $column_value)->first();
        if ($user) {
    ...
    public function verifySmsOtp(Request $request)
    {
        ...
        $code = $validated['code'];
        $otp_exp_sec = BasicSettingsProvider::get()->otp_exp_seconds ?? GlobalConst::DEFAULT_TOKEN_EXP_SEC;
        $auth_column = AgentAuthorization::where('phone', $full_mobile)->where('code', $code)->first();
    ...
    public function resendSmsOtp(Request $request)
    {
        ...
        DB::beginTransaction();
        try {
            $oldToken = AgentAuthorization::where('phone', $full_mobile)->get();
            if ($oldToken) {
                foreach ($oldToken as $token) {
                    $token->delete();
                }
