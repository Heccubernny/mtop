...
class GiftCardController extends Controller
{
    ...
    public function index()
    {
        ...
        $giftCards = GiftCard::auth()->user()->where('status', 1)->latest()->get()->map(function ($item) {
            return [
                ...
                'wallet_currency_rate' => get_amount($item->exchange_rate, null, get_wallet_precision($item->userWallet->currency)),
                'payable_unit_price' => get_amount($item->unit_amount, null, get_wallet_precision($item->userWallet->currency)),
                'payable_charge' => get_amount($item->total_charge, null, get_wallet_precision($item->userWallet->currency)),
                'total_payable' => get_amount($item->total_payable, null, get_wallet_precision($item->userWallet->currency)),
                'status' => $item->status,
            ];
        });
        $data = [
    ...
    public function giftCardDetails()
    {
        ...
        $check_receiver_currency_code = Currency::where('status', true)->where('code', $product_receiver_code)->first();
        ...
        $productCurrency = currency::where('code', $product_receiver_code)->get()->map(function ($data) {
            return [
                ...
                'rate' => get_amount($data->rate, null, get_wallet_precision($data)),
            ];
        });
        $userWallet = UserWallet::with('currency')->where('user_id', auth()->user()->id)->get()->map(function ($data) {
            return [
                'name' => $data->currency->name,
                'balance' => get_amount($data->balance, null, get_wallet_precision($data->currency)),
                'currency_code' => $data->currency->code,
                'currency_symbol' => $data->currency->symbol,
                'currency_type' => $data->currency->type,
                'rate' => get_amount($data->currency->rate, null, get_wallet_precision($data->currency)),
    ...
    public function orderPlace(Request $request)
    {
        ...
        try {
            ...
            $receiver_country = Currency::where('code', $product['recipientCurrencyCode'])->first();
            if (! $receiver_country) {
            ...
            if ($this->basic_settings->sms_notification == true) {
                try {
                    sendSms($user, 'GIFT_CARD', [
                        'title' => $product['productName'] ?? '',
                        'unit_price' => get_amount($charges['card_unit_price'], $charges['card_currency'], $charges['precision_digit']),
                        'qty' => $charges['qty'],
                        'card_total_price' => get_amount($charges['total_receiver_amount'], $charges['card_currency'], $charges['precision_digit']),
                        'charge' => get_amount($charges['total_charge'], $charges['wallet_currency'], $charges['precision_digit']),
    ...
    public function giftCardCharge($form_data, $cardCharge, $userWallet, $sender_country, $receiver_country)
    {
        ...
        $data['card_currency'] = $receiver_country->code;
    ...
    {
        $exchange_rate = get_amount(1, $charges['card_currency']).' = '.get_amount($charges['exchange_rate'], $charges['card_currency'], $charges['precision_digit']);
        ...
        $notification_content = [
            ...
            'email_content' => __('web_trx_id').' : '.$trx_id.'<br>'.__('Card Name').' : '.$data['card_name'].'<br>'.__('receiver Email').': @'.$data['recipient_email'].'<br>'.__('Receiver Phone').' : '.$data['recipient_phone'].'<br>'.__('Card Unit Price').' : '.get_amount($charges['card_unit_price'], $charges['card_currency'], $charges['precision_digit']).'<br>'.__('Card Quantity').' : '.$charges['qty'].'<br>'.__('Card Total Price').' : '.get_amount($charges['total_receiver_amount'], $charges['card_currency'], $charges['precision_digit']).'<br>'.__('Exchange Rate').' : '.$exchange_rate.'<br>'.__('Payable Unit Price').' : '.get_amount($charges['sender_unit_price'], $charges['wallet_currency'], $charges['precision_digit']).'<br>'.__('Payable Amount').' : '.get_amount($charges['conversion_amount'], $charges['wallet_currency'], $charges['precision_digit']).'<br>'.__('Total Charge').' : '.get_amount($charges['total_charge'], $charges['wallet_currency'], $charges['precision_digit']).'<br>'.__('Total Payable Amount').' : '.get_amount($charges['payable'], $charges['wallet_currency'], $charges['precision_digit']).'<br>'.__('Status').' : '.__($status),

            // push notification
            'push_title' => __('Gift Card Order').' ('.userGuard()['type'].')',
            'push_content' => __('web_trx_id').' '.$trx_id.' '.__('Card Name').' : '.$data['card_name'].' '.__('Card Quantity').' : '.$charges['qty'].' '.__('Total Payable Amount').' : '.get_amount($charges['payable'], $charges['wallet_currency'], $charges['precision_digit']),

            // admin db notification
            'notification_type' => NotificationConst::GIFTCARD,
            'admin_db_title' => 'Gift Card Order'.' ('.userGuard()['type'].')',
            'admin_db_message' => 'Transaction ID'.' : '.$trx_id.','.'Card Name'.' : '.$data['card_name'].','.'Card Quantity'.' : '.$charges['qty'].','.'Total Payable Amount'.' : '.get_amount($charges['payable'], $charges['wallet_currency'], $charges['precision_digit']).' ('.$user->email.')',
        ];
