...
{
    use AuthNotifications ,ControlDynamicInputFields,LoggedInUsers,RegisteredUsers;
    ...
    public function login(Request $request)
{
    ...
    if ($user->status == 0) {
        return ApiHelpers::validation([
            'error' => [__('Account has been suspended.')],
        ]);
    }
    ...
    public function register(Request $request)
{
    ...
    $passwordRule = $basic_settings->secure_password
    ...
    $validator = Validator::make($request->all(), [
        ...
        'nin'       => 'nullable|digits:11|required_without:bvn',
    ]);
     ...
     * --------------------------------
     */
    $username = make_username($request->firstname, $request->lastname);
     ...
     * --------------------------------
     */
    $email_verified = !$basic_settings->email_verification;
    ...
    try {
        $user = User::create([
            ...
            'sms_verified'   => false,
            'registered_by'  => GlobalConst::EMAIL,
            'kyc_verified'   => GlobalConst::DEFAULT,
        ...
        /**
         * --------------------------------
         * KYC RECORD (BVN or NIN)
         * --------------------------------
         */
        $user->kyc()->create([
            'type'   => $request->bvn ? 'BVN' : 'NIN',
            'value'  => $request->bvn ?? $request->nin,
     ...
     */
    if ($basic_settings->email_verification) {
        $this->sendEmailVerificationApi($user);
    }
    ...
    return ApiHelpers::success([
        ...
        'success' => [__('Registration successful. Please verify your email and identity.')],
    ]);
}
