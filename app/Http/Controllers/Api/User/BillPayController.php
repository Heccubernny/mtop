...
class BillPayController extends Controller
{
    ...
    public function billPayInfo()
    {
        ...
        try {
            ...
            //     'page' => 0,
            // ], false);
            $billers = (new UtilityHelper())->getBillers('FIXED_BILL_PAYMENT',
        'NG',
        'serviceType'
        0,
        500
        ...
        foreach ($contentArray as &$item) {
            ...
            $item['receiver_currency_rate'] = getAmount(receiver_currency($item['localTransactionCurrencyCode'])['rate'], 8);
    ...
    public function home()
    {
        return response()->json([
            'success' => true,
            'services' => ['airtime', 'data', 'cable_tv']
        ]);
    }
    ...
    public function buyData(Request $request)
    {
        ...
        return DB::transaction(function () use ($request, $user) {
            ...
            $wallet = UserWallet::ngn()->where('user_id', $user->id)->firstOrFail();
            ...
            return response()->json(['success' => true, 'transaction' => $trx]);
        });
    }
    ...
    /* ------------------------------------------------
     | AIRTIME
     ------------------------------------------------ */
    public function airtimeNetworks()
    {
        return response()->json([
            'success' => true,
            'networks' => (new CkHelper())->getMobileNetworks()
    ...
    public function getPackages($providerId)
    {
        return response()->json([
            'success' => true,
            'packages' => (new CkHelper())->getCableTvPacakges($providerId)
        ]);
    }
    ...
    public function verifySmartcard(Request $request)
    {
        ...
        return response()->json(
            (new CkHelper())->verifySmartcard($request->cable, $request->smartcard)
        );
    }
    ...
    public function buyCableTv(Request $request)
    {
        ...
        $provider = CkCableTv::where('code', $request->cable)->firstOrFail();
        $package = CkCableTvPackages::where('package_code', $request->package)->firstOrFail();
    ...
    public function transactions(Request $request)
    {
        return response()->json([
            'success' => true,
            'transactions' => CkTransaction::where('user_id', $request->user()->id)
                ->latest()
                ->paginate(20)
        ]);
    }
...
public function dataServices()
{
    return response()->json([
        'success' => true,
        'services' => [
            ['slug' => 'mtn-data', 'name' => 'MTN Data', 'icon' => asset('images/data/mtn.png')],
            ['slug' => 'airtel-data', 'name' => 'Airtel Data', 'icon' => asset('images/data/airtel.png')],
            ['slug' => 'glo-data', 'name' => 'GLO Data', 'icon' => asset('images/data/glo.png')],
            ['slug' => 'glo-sme-data', 'name' => 'GLO SME Data', 'icon' => asset('images/data/glo.png')],
            ['slug' => 'etisalat-data', 'name' => '9Mobile Data', 'icon' => asset('images/data/9mobile.png')],
        ]
    ]);
}

public function dataPlans($service)
{
    ...
    return response()->json([
        'success' => true,
        'plans' => $response['content']['variations']
    ]);
}

public function buyCableTvs(Request $request, $service)
{
    $request->validate([
    ...
    // Wallet logic should be wrapped in DB::transaction()
    // (reuse your existing wallet + charge helpers)

    $response = CabletvHelper::purchaseTV($service, $request->only(
        'smartcard_number', 'variation_code', 'amount', 'phone'
    ));

    if (!isset($response['content']['transactions'])) {
        return response()->json([
            'success' => false,
            'message' => $response['response_description'] ?? 'Transaction failed'
        ], 400);
    }
...
public function buyDatas(Request $request, $service)
{
    $request->validate([
    ...
    $response = DataSubscriptionHelper::purchaseData($service, $request->only(
        'phone_number', 'variation_code', 'amount'
    ));
