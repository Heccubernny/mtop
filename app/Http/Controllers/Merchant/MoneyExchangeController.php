...
class MoneyExchangeController extends Controller
{
    public function index(Request $request)
    {
        ...
        $user_wallets = MerchantWallet::where('merchant_id', auth()->user()->id)->whereHas('currency', function ($q) {
            $q->where('status', GlobalConst::ACTIVE);
        })->with('currency:id,code,name,flag,rate,type,symbol,country')->get();
        $transactions = Transaction::merchantAuth()->where(['type' => PaymentGatewayConst::TYPEMONEYEXCHANGE])->latest()->take(10)->get();
        $charges = TransactionSetting::where('slug', 'money_exchange')->first();

        return view('merchant.sections.money-exchange.index', compact('page_title', 'user_wallets', 'charges', 'transactions'));
    }
    ...
    public function moneyExchangeSubmit(Request $request)
    {
        ...
        $user_from_wallet = MerchantWallet::where('merchant_id', auth()->user()->id)->whereHas('currency', function ($q) use ($validated) {
            $q->where('code', $validated['exchange_from_currency']);
        })->first();
        if (! $user_from_wallet) {
            return back()->with(['error' => ['From wallet('.$validated['exchange_from_currency'].') doesn\'t exists']]);
        }
        $user_to_wallet = MerchantWallet::where('merchant_id', auth()->user()->id)->whereHas('currency', function ($q) use ($validated) {
            $q->where('code', $validated['exchange_to_currency']);
        })->first();
        if (! $user_to_wallet) {
            return back()->with(['error' => ['To exchange wallet('.$validated['exchange_to_currency'].') doesn\'t exists']]);
        }
        $charges = TransactionSetting::where('slug', 'money_exchange')->first();
        if (! $charges) {
            return back()->with(['error' => [__("Exchange money isn't available right now")]]);
        }
        ...
        // daily and monthly
        try {
            (new TransactionLimit)->trxLimit('merchant_id', $user_from_wallet->merchant->id, PaymentGatewayConst::TYPEMONEYEXCHANGE, $user_from_wallet->currency, $validated['exchange_from_amount'], $charges, PaymentGatewayConst::SEND);
        } catch (Exception $e) {
        ...
        if ($chargeCalculate->payable > $chargeCalculate->from_wallet_balance) {
            return back()->with(['error' => [_('Your Wallet Balance Is Insufficient')]]);
        }
        $trx_id = 'ME'.getTrxNum();
        // Transaction Start
        DB::beginTransaction();
        try {
            $inserted_id = DB::table('transactions')->insertGetId([
                ...
                'merchant_wallet_id' => $user_from_wallet->id,
            ...
            // Push Notifications
            try {
                if ($basic_setting->merchant_push_notification == true) {
                    // push notification
