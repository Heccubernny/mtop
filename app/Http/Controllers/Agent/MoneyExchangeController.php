...
class MoneyExchangeController extends Controller
{
    public function index(Request $request)
    {
        ...
        $user_wallets = AgentWallet::where('agent_id', auth()->user()->id)->whereHas('currency', function ($q) {
            $q->where('status', GlobalConst::ACTIVE);
        })->with('currency:id,code,name,flag,rate,type,symbol,country')->get();
        $transactions = Transaction::agentAuth()->where(['type' => PaymentGatewayConst::TYPEMONEYEXCHANGE])->latest()->take(10)->get();
        $charges = TransactionSetting::where('slug', 'money_exchange')->first();

        return view('agent.sections.money-exchange.index', compact('page_title', 'user_wallets', 'charges', 'transactions'));
    }
    ...
    public function moneyExchangeSubmit(Request $request)
    {
        ...
        $user_from_wallet = AgentWallet::where('agent_id', auth()->user()->id)->whereHas('currency', function ($q) use ($validated) {
        ...
        $user_to_wallet = AgentWallet::where('agent_id', auth()->user()->id)->whereHas('currency', function ($q) use ($validated) {
        ...
        // daily and monthly
        try {
            (new TransactionLimit)->trxLimit('agent_id', $user_from_wallet->agent->id, PaymentGatewayConst::TYPEMONEYEXCHANGE, $user_from_wallet->currency, $validated['exchange_from_amount'], $charges, PaymentGatewayConst::SEND);
        } catch (Exception $e) {
        ...
        try {
            $inserted_id = DB::table('transactions')->insertGetId([
                ...
                'agent_wallet_id' => $user_from_wallet->id,
            ...
            // Push Notifications
            try {
                if ($basic_setting->agent_push_notification == true) {
                    // push notification
