...
class SendMoneyController extends Controller
{
    ...
    public function index()
    {
        ...
        return view('agent.sections.send-money.index', compact('page_title', 'sendMoneyCharge', 'transactions', 'receiver_wallets', 'sender_wallets'));
    }
    ...
    public function confirmedSendMoney(Request $request)
    {
        ...
        $receiver = Agent::notAuth()->where(function ($query) use ($credentials) {
            $query->where('email', $credentials)
                ->orWhere('mobile', (int) $credentials)
                ->orWhere('full_mobile', $credentials);
        })->active()->first();
        ...
        $charges = $this->transferCharges($validated['sender_amount'], $trx_charges, $sender_wallet, $receiver_wallet->currency);
            ...
            if ($sender) {
                ...
                try {
                    if ($basic_setting->agent_email_notification == true) {
                        $notifyDataSender = [
                            'trx_id' => $trx_id,
                            'title' => __('Send Money to').' @'.@$receiver_wallet->agent->username.' ('.@$receiver_wallet->agent->fullname.')',
            ...
            if ($receiverTrans) {
                ...
                try {
                    ...
                    if ($basic_setting->agent_email_notification == true) {
                        $notifyDataReceiver = [
                            'trx_id' => $trx_id,
                            'title' => __('Received Money from').' @'.@$sender_wallet->agent->username.' ('.@$sender_wallet->agent->fullname.')',
                            'received_amount' => get_amount($charges['receiver_amount'], $charges['receiver_currency'], $charges['r_precision_digit']),
                            'status' => __('success'),
                        ];
                        // send notifications
