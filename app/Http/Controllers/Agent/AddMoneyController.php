...
{
    use FlutterwaveTrait,Manual,RazorTrait,SslcommerzTrait,Stripe;
    ...
    public function success(Request $request, $gateway)
    {
        ...
        if (! $checkTempData) {
            return redirect()->route('agent.add.money.index')->with(['error' => [__("Transaction Failed. The record didn't save properly. Please try again")]]);
        }
    ...
    public function coinGateSuccess(Request $request, $gateway)
    {
        ...
        try {
            ...
            if (Transaction::where('callback_ref', $token)->exists()) {
                if (! $checkTempData) {
                    return redirect()->route('agent.add.money.index')->with(['success' => [__('Transaction request sended successfully!')]]);
                }
            } else {
                if (! $checkTempData) {
                    return redirect()->route('agent.add.money.index')->with(['error' => [__("Transaction Failed. The record didn't save properly. Please try again")]]);
