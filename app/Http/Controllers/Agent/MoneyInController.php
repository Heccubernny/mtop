...
class MoneyInController extends Controller
{
    ...
    public function index()
    {
        ...
        return view('agent.sections.money-in.index', compact('page_title', 'moneyInCharge', 'transactions', 'receiver_wallets', 'sender_wallets'));
    }
    ...
    public function adminNotification($trx_id, $charges, $sender, $receiver)
    {
        $notification_content = [
            ...
            'admin_db_message' => 'Sender'.': @'.$sender->email.','.'Receiver'.': @'.$receiver->email.','.'Sender Amount'.' : '.get_amount($charges['sender_amount'], $charges['sender_currency'], $charges['precision_digit']).','.'Receiver Amount'.' : '.get_amount($charges['receiver_amount'], $charges['r_precision_digit'], $charges['precision_digit']),
        ];
