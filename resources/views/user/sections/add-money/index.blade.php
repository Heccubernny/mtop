                                ...
                                <div class="row">
                                    ...
                                    <div class="col-xl-6 col-lg-6 form-group">
                                        ...
                                        <select class="form--control nice-select gateway-select" name="currency">
                                            @forelse ($payment_gateways_currencies ?? [] as $item)
                                                <option data-currency="{{ $item->currency_code }}"
                                                    ...
                                                    data-currency-id="{{ $item->id }}" value="{{ $item->alias }}">
                                                    {{ $item->name }}
                                                </option>
                                            @empty
                                                <option value="null" disabled selected>{{ __('No Gateway Available') }}
                                                </option>
                                            @endforelse
                                        </select>
                                    </div>
                                    <div class="col-xl-6 col-lg-6 form-group">
                                        ...
                                        <div class="input-group">
                                            <input class="form--control number-input" name="amount" type="text"
                                                ...
                                                required>
                                            <select class="form--control nice-select wallet-currency"
                                                ...
                                                @foreach ($wallet_currencies ?? [] as $data)
                                                    <option data-rate="{{ $data->rate }}"
                                                        ...
                                                        value="{{ $data->code }}"
                                                        {{ $data->default ? 'selected' : '' }}>{{ $data->code }}
                                                    </option>
                                                @endforeach
                                            </select>
                                        </div>
                                    ...
                                    <div class="col-xl-12 col-lg-12 form-group">
                                        <div class="note-area">
                                            ...
                                            <code class="d-block text--dark fw-bold balance-show mt-10 text-end">--</code>
                                        </div>
                                    </div>
                                    <div class="col-xl-12 col-lg-12">
                                        <button class="btn--base w-100 btn-loading addMoneyBtn"
                                            type="submit">{{ __('Add Money') }} <i
            ...
            <div class="dashboard-header-wrapper">
                ...
                <div class="dashboard-btn-wrapper">
                    <div class="dashboard-btn mb-2">
                        <a class="btn--base"
                            href="{{ setRoute('user.transactions.index', 'add-money') }}">{{ __('View More') }}</a>
                    </div>
                </div>
            </div>
...
@push('script')
    <script>
        ...
        function acceptVar() {
            ...
            var walletCurrencyType = $("select[name=wallet_currency] :selected").data('type');
        ...
        function getExchangeRate() {
            ...
            var gCurrencyRate = parseFloat(acceptVar().currencyRate).toFixed(presion);
            var wCurrencyCode = acceptVar().wCurrencyCode;
            var wCurrencyRate = parseFloat(acceptVar().wCurrencyRate).toFixed(acceptVar().wPrecison);
            var exchange_rate = parseFloat(gCurrencyRate / wCurrencyRate).toFixed(presion);
            $('.rate-show').html("1 " + gCurrencyCode + " = " + parseFloat(wCurrencyRate).toFixed(presion) + " " +
                wCurrencyCode);
        ...
        function feesCalculation() {
            ...
            if ($.isNumeric(percent_charge) && $.isNumeric(fixed_charge) && $.isNumeric(sender_amount)) {
                ...
                var percent_charge_calc = (parseFloat(sender_amount) / 100) * parseFloat(percent_charge) * parseFloat(
                    exchange_rate);
        ...
        // function feesCalculation() {
        //     let exchange_rate = parseFloat(getExchangeRate());
        //     let sender_amount = parseFloat($("input[name=amount]").val());

        //     if (isNaN(sender_amount) || sender_amount <= 0) {
        //         sender_amount = 0;
        //     }

        //     // Currency-specific charges
        //     let fixed_charge = parseFloat(acceptVar().currencyFixedCharge);
        //     let percent_charge = parseFloat(acceptVar().currencyPercentCharge);

        //     if (
        //         $.isNumeric(exchange_rate) &&
        //         $.isNumeric(sender_amount) &&
        //         $.isNumeric(fixed_charge) &&
        //         $.isNumeric(percent_charge)
        //     ) {
        //         /*
        //          | Convert fixed charge to selected currency
        //          | If your fixed charge is stored in USD
        //          */
        //         let fixed_charge_calc = fixed_charge * exchange_rate;

        //         /*
        //          | Percent fee based on sender amount
        //          */
        //         let percent_charge_calc = (sender_amount * percent_charge) / 100;

        //         let total_charge = fixed_charge_calc + percent_charge_calc;

        //         return {
        //             total: total_charge.toFixed(presion),
        //             fixed: fixed_charge_calc.toFixed(presion),
        //             percent: percent_charge
        //         };
        //     }
        ...
        function getFees() {
            ...
            $(".fees-show").html("{{ __('charge') }}: " + parseFloat(charges.fixed).toFixed(presion) + " " +
                gCurrencyCode + " + " + parseFloat(charges.percent).toFixed(presion) + "%");
        }
        ...
        // function getPreview() {
        //     let senderAmount = parseFloat($("input[name=amount]").val());
        //     let wCurrencyCode = acceptVar().wCurrencyCode;

        //     if (isNaN(senderAmount) || senderAmount <= 0) {
        //         senderAmount = 0;
        //     }

        //     // Sending Amount
        //     $('.request-amount').text(
        //         senderAmount.toFixed(acceptVar().wPrecison) + " " + wCurrencyCode
        //     );

        //     // Fees (already in selected currency)
        //     let charges = feesCalculation();
        //     let totalFees = charges ? parseFloat(charges.total) : 0;

        //     $('.fees').text(
        //         totalFees.toFixed(presion) + " " + wCurrencyCode
        //     );

        //     // Will Get (same currency)
        //     $('.will-get').text(
        //         senderAmount.toFixed(acceptVar().wPrecison) + " " + wCurrencyCode
        //     );

        //     // ✅ Pay In Total (NO exchange rate here)
        //     let pay_in_total = senderAmount + totalFees;

        //     $('.pay-in-total').text(
        //         pay_in_total.toFixed(presion) + " " + wCurrencyCode
        //     );
        // }
            ...
            $.ajax({
                ...
                success: function(response) {
                    $('.balance-show').html("{{ __('Available Balance') }}: " + parseFloat(response.data)
                        .toFixed(acceptVar().wPrecison) + " " + senderCurrency);
                }
            });
        }
