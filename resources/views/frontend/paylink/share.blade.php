            ...
            <div class="payment-preview-box">
                ...
                <div class="payment-preview-box-left">
                    ...
                    <div class="form-group">
                        ...
                        <div class="input-group">
                            <div class="input-group-prepend">
                                <span class="input-group-text">{{ @$payment_link->currency }}</span>
                            </div>
                    ...
                    @if ($payment_link->type ==  payment_gateway_const()::LINK_TYPE_PAY)
                        <div class="payment-preview-thumb">
                            ...
                            @else
                                <img src="{{ asset('frontend/images/logo/link_icon.png') }}" alt="Link Image">
                            @endif
                        </div>
                    @endif
                </div>
                <div class="payment-preview-box-right">
                    ...
                    <div class="row mb-30-none">
                        ...
                        @auth('web')
                            <div class="col-xl-12 mb-30 {{ $payLink->wallet_status === false ? 'd-none':'' }}">
                                <label>{{ __('Pay With')}} {{ @$basic_settings->site_name??"QRPAY" }} {{ __('wallet Balance') }}</label>
                                <div class="payment-form-area">
                                    <div class="payment-form-wrapper">
                                        <div class="paylink-radio-wrapper">
                                            @foreach ($qrpay_wallets ?? [] as $wallet)
                                                <div class="paylink-radio-item">
                                                    <input type="radio" id="radio-wallet-{{ $wallet->code }}" name="wallet_currency" value="{{ $wallet->code }}">
                                                    <label for="radio-wallet-{{ $wallet->code }}">{{ $wallet->code }}</label>
                                                </div>
                                            @endforeach
                                        </div>
                                    </div>
                                        ...
                                        <div class="payment-form-wrapper">
                                            <div class="paylink-radio-wrapper">
                                                <div class="paylink-radio-item">
                                                    ...
                                                    <label for=""><img src="{{ get_fav($basic_settings) }}" alt="{{ @$basic_settings->site_name ?? "QRPAY" }}"></label>
                                                </div>
                                            </div>
                                        </div>
                                    </div>
...
@push("script")
    ...
    <script>
        ...
        $('input[name="payment_gateway"]').on('click', function(){
            $('#payment-form input[name="wallet_currency"]').prop('checked', false);
            $('#payment-form input[name="card_system"]').prop('checked', false);
            $('.card-fields').addClass('d-none');
        })
        $('input[name="wallet_currency"]').on('click', function(){
        ...
        $('#submit-button').on('click', function () {
            ...
            let wallet_currency = $('#payment-form input[name="wallet_currency"]:checked').val();
            alert
            var form = document.getElementById('payment-form');
            event.preventDefault();

            if(payemnt_gateway == undefined && wallet_currency == undefined){
                ...
                form.submit();
            }else if(wallet_currency != undefined ){
